...
Item {
  ...
  readonly property string collectorPath: pluginDir + "/bin/omarchy-agent-usage-commandcode"
  ...
  Process {
    ...
    onExited: function(exitCode) {
      ...
      if (exitCode !== 0 || stderr.text.trim()) {
        console.warn("commandcode-usage", exitCode !== 0 ? "exit " + exitCode : stderr.text.trim())
      }
    }
